...
public final class DisjointSets<T> {
  ...
  // every index points to parent of that index. if value is negative, that
  // index is a set root, and the absolute
  // value is the rank
   ...
   * 
   * @param x An element in the first set to union
   * @param y An element in the second set to union
   */
  ...
  /**
   * Returns the full collection of elements in the same set as the given
   * elements. Includes the given element in the returned collection.
   * 
   * @param e the element to find the owning set of
  ...
  public Collection<T> retrieveSet(T e) {
    ...
    ObjectIterator<Object2IntMap.Entry<T>> it = _elements.object2IntEntrySet()
        .fastIterator();
    while (it.hasNext()) {
  ...
  /**
   * Returns a collection of all the sets, where each set is a collection of all
   * the values in the set.
   * 
   * @return a collection of all the sets, where each set is a collection of all
   *         the values in the set
   */
  public Collection<? extends Collection<T>> retrieveSets() {
    Int2ObjectMap<ArrayList<T>> sets = new Int2ObjectOpenHashMap<>(_numSets,
        1f);
