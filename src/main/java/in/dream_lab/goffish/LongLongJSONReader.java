...
/* TODO: Create a generic reader class, which allows other classes to define
 * custom value type for vertex and edge.
 * Reads the graph from JSON format
 * [srcid,srcvalue, [[sinkid1,edgeid1,edgevalue1], [sinkid2,edgeid2,edgevalue2] ... ]]
 */

public class LongLongJSONReader<S extends Writable, V extends Writable, E extends Writable, K extends Writable, M extends Writable>
    implements
  ...
  Vertex<V, E, LongWritable, LongWritable> createVertex(String JSONString) {
    ...
    Vertex<V, E, LongWritable, LongWritable> vertex = new Vertex<V, E, LongWritable, LongWritable>(
        sourceID);
    V value = (V) new LongWritable(Long.parseLong(JSONInput.get(1).toString()));
    ...
    for (Object edgeInfo : edgeList) {
      ...
      E edgeValue = (E) new LongWritable(Long.parseLong(edgeValues[2].toString()));
